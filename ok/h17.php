...
<form method="POST" id="f17" action="<?php echo $_SERVER['PHP_SELF']; ?>">
	rec: <input type="text" name="rec"><br>
	roll: <input type="text" name="roll"><br>
	definicija: <input type="text" name="df"><br>
	napomena: <textarea name="np"></textarea><br>
	frm: <input type="text" name="frm"><br>
	jpg: <input type="text" name="jpg"><br>
	<input type="submit" value="pucaj!">
</form>
<br>
...
mysqli_set_charset($konekcija, 'utf8'); 
	...
	$upit4="INSERT INTO roll_c(rec,roll,opis_n,opis_e,frm,img,tip) VALUES ($rec,$roll,'$df','$np',$frm,$jpg,1)" . ";";
	mysqli_query($konekcija,$upit4);
	//echo "<br>" . $upit4 . "<br>";
/*
for ($ii=1215; $ii<1432; ++$ii) {
	$upit1="SELECT * FROM roll_c WHERE id=" . $ii . ";";
	$rezultat = mysqli_query($konekcija,$upit1);
	$red1 = mysqli_fetch_assoc($rezultat);
	$upit2="SELECT * FROM roll_s where rec=" . $red1['rec'] . " AND roll=" . $red1['roll'] . ";";
	$rezultat2 = mysqli_query($konekcija,$upit2);
	$red2 = mysqli_fetch_assoc($rezultat2);
	$diff=$red2['diff'];

	$upit3="UPDATE roll_c SET img=" . ($red1['frm'] + $diff) . " where id=" . $ii . ";";
	mysqli_query($konekcija,$upit3);
	echo $upit3 . "<br>";
}
*/
?>
...
while($red1 = mysqli_fetch_assoc($rezultat)) {
	$ii=strpos($red1['opis_e'], "etni");
	$jj=strpos($red1['opis_n'], "etni");
	$kk=strpos($red1['opis_e'], "DM");
	$ll=strpos($red1['opis_n'], "DM");
	$mm=strpos($red1['opis_e'], "ihailov");
	$nn=strpos($red1['opis_n'], "ihailov");
	$oo=strpos($red1['opis_e'], "D.M");
	$pp=strpos($red1['opis_n'], "D.M");
	$atresa="/NARA/T316.php?rec=" . $red1['rec'] . "&roll=" . $red1['roll'] . "&broj=" . $red1['img'];
	$a_tag="<a href=" . chr(34) . $atresa . chr(34) . " target=new>";
	if($ii>0 or $jj>0 or $kk>0 or $ll>0 or $mm>0 or $nn>0 or $oo>0 or $pp>0) {echo "<u>". $a_tag . $red1['rec'] . "-" . $red1['roll'] . ", " . $red1['frm'] . ":</a></u> " . $red1['opis_n'] . "; " . $red1['opis_e'] . "<br><br>";}
	++$rr;
	}
echo $rr;
die;
