...
while($red1 = mysqli_fetch_assoc($rezultat)) {
	$ii=$red1['id'];
	$nn=$red1['opis_n'];
	$nn=str_replace("<br><br>","<br>",$nn);
	//$nn=$nn . "<br>";
	echo $nn;
	//echo strlen($nn) . " : " . $nn . "<br>";
	//if (substr($nn,strlen($nn)-8,8)=="<br><br>" ) {echo "..!!!..";}
	$upit2="UPDATE roll_c SET opis_n='$nn' WHERE id=$ii";
	//echo $upit2;
	$rezultat2 = mysqli_query($konekcija,$upit2);
	}
die;
...
while($red1 = mysqli_fetch_assoc($rezultat)) {
	...
	if($ii>0 or $jj>0 or $kk>0 or $ll>0 or $mm>0 or $nn>0 or $oo>0 or $pp>0) {
		echo "<u>". $a_tag . $red1['rec'] . "-" . $red1['roll'] . ", " . $red1['frm'] . ":</a></u> " . $red1['opis_n'] . "; " . $red1['opis_e'] . "<br><br>";
		++$rr;
		}
	}
echo $rr;
