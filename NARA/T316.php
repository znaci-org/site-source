...
class unit_att
{
	...
	function pos_determin($unitt,$konekcija) {
		$upit_rx="SELECT distinct(unit) FROM roll_s WHERE recx=" . $this->rec_determin($unitt,$konekcija) . " ORDER BY unit";
		//echo $upit_rx . "!!!<br>";
...
$upit2="SELECT * FROM roll_c WHERE rec=" . $rec . " AND roll=" . $roll . " AND tip=0" . " ORDER BY frm;";
//echo $upit2;//
...
Dostupno ukupno kompletnih rolni listova dokumenata (total rolls): <b class=bigbigTitle_c><?php echo $totalnotes; ?></b><b class=bigTitle> - sa ukupno listova dokumenata (total pages): </b><b class=bigbigTitle_c><?php echo $totalpages; ?></b></p>

<title><?php echo $naziv; ?> (<?php echo $naziv2; ?>) 
</title>
...
$upit="Select distinct(unit) from roll_s where recx=" . $kl_num[$ii] . " order by unit;";
//echo $upit;
...
for ($ii=1;$ii<$broj_poglavlja+1;$ii++) {
	...
	$spx='<tr><td vAlign=top ' . $boja[$ii] . '> <a href=' . chr(34) . $_SERVER['PHP_SELF'] . "?rec=" . $rec . '&broj=' . $img[$ii-1] . '&roll=' . $roll . '"' . '><b>' . $opis_n[$ii-1] . ' - ' . $opis_e[$ii-1] . ', ' . $dt1[$ii-1] . ' - ' . $dt2[$ii-1] . '</b></a>' . $sp6 . $detalj[$ii-1] . $sp4 . $frm[$ii-1] . $slika[$ii-1] . $sp5;
	echo $spx;
	echo "\n";
}
?>
