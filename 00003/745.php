<meta charset="utf-8">

<!DOCTYPE html>

<?php

function into_roman($argm) {
$roem_x = array(
    'M' => 1000,
    'CM' => 900,
    'D' => 500,
    'CD' => 400,
    'C' => 100,
    'XC' => 90,
    'L' => 50,
    'XL' => 40,
    'X' => 10,
    'IX' => 9,
    'V' => 5,
    'IV' => 4,
    'I' => 1,
);
$roem_ix=array('M','CM','D','CD','C','XC','L','XL','X','IX','V','IV','I');
	$rr=0;
	$rzlt="";
	while (1) {
		if($argm<1) {break;}
		if($argm>=$roem_x[$roem_ix[$rr]]) {
			$argm=$argm - $roem_x[$roem_ix[$rr]];
			$rzlt= $rzlt . $roem_ix[$rr];
		} else {
			$rr++;
		}
	}
	return $rzlt;
}

$knjige=array('knjiga I'=>745,'knjiga II'=>746,'knjiga III'=>747);
if (isset($_GET['pripadnost'])) {
	$tmpm=$_GET['pripadnost'];
	$broj_knjige = $knjige[$tmpm];
} else {
	$broj_knjige = 745;
	$tmpm='knjiga I';
}

$konekcija = mysqli_connect("", getenv(MYSQL_UN), getenv(MYSQL_PW), getenv(MYSQL_DB));
mysqli_set_charset($konekcija, 'utf8'); 

if (mysqli_connect_errno()) {
	echo "Ne mogu da se povežem sa bazom. ";
	exit();
} 
$rr=0;
$iiix=0;
$maxx=array(745=>538,746=>614,747=>369);
//$br_knj=744;
$autor="Vladimir Dedijer";
$nasl_knj="DNEVNIK 1941-1944";
if ($_GET['broj']) {
	$ova_strana = $_GET['broj'];
	$iiix=2;
} else {
	$ova_strana = mt_rand(5,$maxx[$broj_knjige]);
}
$niz_s="";
$niz_o="";
$nasl="";
$upitt="SELECT * FROM sadrzaji WHERE knjiga=" . $broj_knjige . " ORDER BY id";
$rezultat = mysqli_query($konekcija,$upitt);
$red = mysqli_fetch_assoc($rezultat);

$naslx=$red['naslov'];
$niz_strana[$rr]=$red['strana_pdf'];
$niz_oznaka[$rr]=$red['strana'];
if ($red['autor']) {
	$naslov[$rr]=$red['autor'] . ": <i>" . $red['naslov'] . "</i>";
} else {
	$naslov[$rr]=$red['poglavlje'];
}
while ($red) {
	$red = mysqli_fetch_assoc($rezultat);
	$rr++;
	$niz_strana[$rr]=$red['strana_pdf'];
	$niz_oznaka[$rr]=$red['strana'];
	if ($red['autor']) {
		$naslov[$rr]=$red['autor'] . ": <i>" . $red['naslov'] . "</i>";
	} else {
		$naslov[$rr]=$red['poglavlje'];
	}
}

for ($ii=1;$ii<500;$ii++) {
	if (!$niz_strana[$ii]) {break;}
}
$broj_poglavlja=$ii;
if ($iiix!=2) {$ova_strana = mt_rand(5,$maxx[$broj_knjige]);}

if ($ova_strana > $maxx[$broj_knjige]) { $ova_strana = $maxx[$broj_knjige]; }
if ($ova_strana < 1) { $ova_strana = 1; }

for ($ii=1;$ii<$broj_poglavlja+1;$ii++) {$boja[$tt]="";}
for ($ii=1;$ii<$broj_poglavlja+1;$ii++) {
if ($ova_strana > $niz_strana[$ii-1] - 1) {$tt=$ii;}
}

$boja[$tt]=" class=srednjezeleno";
$sp4='</td><td align=right>';
$sp5='</td></tr>';

/*for ($ii=0;$ii<$broj_poglavlja;$ii++) {
	$upit="INSERT INTO sadrzaji (poglavlje,strana,strana_pdf) VALUES ('$naslov[$ii]',$niz_oznaka[$ii],$niz_strana[$ii]);";
	$uspex=mysqli_query($konekcija,$upit);
	echo $upit . "###" . $uspex . '<br>';
}*/
$aaa=$_SERVER['PHP_SELF'];
$aa="<center><form method=" . chr(34) . "GET" . chr(34) . " id=" . chr(34) . "f1" . chr(34) . " action=" . chr(34) . $aaa . chr(34) . "><table><tr>";
foreach($knjige as $x=>$x_value) {
	$rrx++;
	if($x_value==$broj_knjige) {$ddtk=" class=dubokozeleno";} else {$ddtk="";}
	$aa = $aa . "<td" . $ddtk . "><input type=" . '"' . "submit" . '"' . " name=" . '"' . "pripadnost" . '"' . " value=" . '"knjiga ' . into_roman($rrx) . '"' . "><font size=" . '"' . "4" . '"' . "></font></td>\n";
}
$aa=$aa . "</tr></table></form></center>";
?>
<html>
<head>
<meta http-equiv="Content-Type" content="text/html; charset=utf-8">
<title><?php echo $autor . ": " . $nasl_knj; ?>
</title>
<SCRIPT LANGUAGE="JavaScript">
<!--
function freg(smer){
	var f2_val;
	f2_val=document.getElementById('d2').value;
	if (smer==0) { 
		--f2_val; }
	if (smer==1) { 
		++f2_val; }
	if (f2_val<1) { 
		f2_val=1; }
	document.getElementById('d2').value = f2_val;
	document.getElementById("f2").submit();
}
-->
</SCRIPT>
<style type="text/css">
<!--
@import url(/normal.css);
-->
</style>
</head>
<body>
<br><b class=bigTitle><?php echo $autor . ": " . $nasl_knj . ", knjiga "  . into_roman($broj_knjige - 744); ?>
</b>
<br>
<div style="text-align: center;">

<div class=BELITASTER>
<form name="input1" action="<?php echo $_SERVER[PHP_SELF]; ?>" method="get" id='f2'>
<table width="100%"  border=1>
<tr><td class=svetloplavo><b>naslov</b></td><td align=right class=svetloplavo><b>broj stranice</b></td><td align="center" class=svetloplavo><b>slika stranice<br /><CENTER><TABLE border="1">
<TR><TD><input type="button" value="<<" name="previous" onclick="freg(0);">
</TD><TD><b>page no: (1-<?php echo $maxx[$broj_knjige];?>)</b>: <input type="text" name="broj" id='d2' value=<?php echo $ova_strana;?>>
</TD><TD><input type="button" value=">>" name="next" onclick="freg(1);">
<TD rowspan=2><a href="/" target="new"><b>znaci.org</b></a>
<tr><td colspan=3 align=center><input type="submit" value="go_to">
</TD></TR>
</TABLE></CENTER></b></td></tr>
<input type="hidden" name="pripadnost" value="<?php echo $tmpm;?>">
</form>

<?php
for ($ii=0;$ii<$broj_poglavlja+1;$ii++) {
	$spx='<tr><td' . $boja[$ii] . '> <a href=' . '"' . '745' . '.php?broj=' . $niz_strana[$ii-1] . '&pripadnost=' . 'knjiga '  . into_roman($broj_knjige - 744) . '"' . '>' . $naslov[$ii-1] . $sp4 . $niz_oznaka[$ii-1] . $sp5;
	if ($ii==0) {
		echo '<tr><td class=tamnozeleno colspan=2>' . $aa . '</td><td vAlign=' . '"' . 'top' . '"' . ' rowspan=' . ($broj_poglavlja+1) . ' width=' . '"' . '60%' . '"' . ' height=1000><center><iframe src=' . '"' . '/00003/' . $broj_knjige . '.pdf#page=' . $ova_strana . '"' . ' style=' . '"' . 'width:100%; height:1000; overflow-x:hidden;' . '"' . '></iframe></td></tr>';
	} else {
		echo $spx;
	}
	echo "\n";
}
?>
</TABLE>
</div>

</center>
</body>
</html>
