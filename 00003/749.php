...
<?php
for ($ii=1;$ii<$broj_poglavlja+1;$ii++) {
	$spx='<tr><td' . $boja[$ii] . '> <a href=' . '"' . '749.php?broj=' . $niz_strana[$ii-1] . '&go!=go!' . '"' . '>' . $naslov[$ii-1] . $sp4 . $niz_oznaka[$ii-1] . $sp5;
	if ($ii==1) {
		echo substr($spx,0,strlen($spx)-5) . '<td vAlign=' . '"' . 'top' . '"' . ' rowspan=' . $broj_poglavlja . ' width=' . '"' . '60%' . '"' . ' height=1000><center><iframe src=' . '"' . '/00003/749.pdf#page=' . $ova_strana . '"' . ' style=' . '"' . 'width:100%; height:1000; overflow-x:hidden;' . '"' . '></iframe></td></tr>';
	} else {
		echo $spx;
	}
