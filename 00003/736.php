...
$maxx=436;
$br_knj=736;
$autor="Savez antifa&#353;isti&#269;kih boraca Hrvatske";
$nasl_knj="RU&#352;ENJE ANTIFA&#352;ISTI&#268;KIH SPOMENIKA U HRVATSKOJ 1990–2000.";
if ($_GET['broj']) {
...
if ($ova_strana > $niz_oznaka[$ii-1] - 1) {$tt=$ii;}
}
...
for ($ii=1;$ii<$broj_poglavlja+1;$ii++) {
	...
	$spx='<tr><td' . $boja[$ii] . '> <a href=' . '"' . $br_knj . '.php?broj=' . $niz_oznaka[$ii-1] . '"' . '>' . $bold1 . $bold3 . $naslov[$ii-1] . $bold4 . $bold2 . $sp4 . $niz_strana[$ii-1] . $sp5;
	if ($ii==1) {
		echo substr($spx,0,strlen($spx)-5) . '<td align=' . '"' . 'center' . '"' . 'vAlign=' . '"' . 'top' . '"' . ' rowspan=' . $broj_poglavlja . ' width=' . '"' . '80%' . '"' . '><center><img src=' . '"' . '/images/736/736 - ' . nulle($ova_strana) . '.jpg' . '"' . ' style=' . '"' . 'width:100%; ' . '"' . '></td></tr>';
	} else {
		echo $spx;
	}
