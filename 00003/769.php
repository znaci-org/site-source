...
$maxx=401;
$br_knj=769;
$autor="Srečko Vilhar, Albert Klun";
$nasl_knj="PRVA IN DRUGA PREKOMORSKA BRIGADA";
if ($_GET['broj']) {
...
for ($ii=1;$ii<$broj_poglavlja+1;$ii++) {
	...
	$spx='<tr><td' . $boja[$ii] . '> <a href=' . '"' . $br_knj . '.php?broj=' . $niz_strana[$ii-1] . '"' . '>' . $bold1 . $bold3 . $naslov[$ii-1] . $bold4 . $bold2 . $sp4 . $niz_strana[$ii-1] . $sp5;
	if ($ii==1) {
		echo substr($spx,0,strlen($spx)-5) . '<td vAlign=' . '"' . 'top' . '"' . ' rowspan=' . $broj_poglavlja . ' width=' . '"' . '60%' . '"' . ' height=2000><center><iframe src=' . '"' . '/00003/' . $br_knj . '.pdf#page=' . $ova_strana . '"' . ' style=' . '"' . 'width:100%; height:2000; overflow-x:hidden;' . '"' . '></iframe></td></tr>';
	} else {
		echo $spx;
	}
